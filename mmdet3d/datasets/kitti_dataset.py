...
from ..core.visualizer import show_multi_modality_result, show_result
...
class KittiDataset(Custom3DDataset):
    ...
    def evaluate(self,
        ...
        from mmdet3d.core.evaluation import Evaluation as kitti_eval
        gt_annos = [info['annos'] for info in self.data_infos]
        evaluation= kitti_eval(gt_annos)
        results = evaluation.evaluate(
        gt_annotations=gt_annos,
        dt_annotations=result_files,
        current_class=self.CLASSES)

        results['entire_area']['mAP'] = (results['entire_area']['Car_3d_all'] + results['entire_area']['Pedestrian_3d_all'] + results['entire_area']['Cyclist_3d_all']) / 3
        results['roi']['mAP'] = (results['roi']['Car_3d_all'] + results['roi']['Pedestrian_3d_all'] + results['roi']['Cyclist_3d_all']) / 3
        print("Results: \n"
    ...
    def bbox2result_kitti(self,
        ...
        for idx, pred_dicts in enumerate(
            ...
            if submission_prefix is not None:
                curr_file = f'{submission_prefix}/{sample_idx:05d}.txt' #VOD改
    ...
    def bbox2result_kitti2d(self,
        ...
        if submission_prefix is not None:
            ...
            for i, anno in enumerate(det_annos):
                sample_idx = self.data_infos[i]['image']['image_idx']
                cur_det_file = f'{submission_prefix}/{sample_idx:05d}.txt' #VOD 改？
